...
import { SortDirection } from './clients-sortable.directive';
 ...
 */
function sort(data: Clients[], column: string, direction: string): Clients[] {
    if (direction === '') {
        return data;
    } else {
 ...
 */
function matches(data: Clients, term: string, pipe: PipeTransform) {
    return data.id?.toString().toLowerCase().includes(term)
        || data.name?.toLowerCase().includes(term)
        || data.postal_address?.toLowerCase().includes(term)
        || data.street?.toLowerCase().includes(term)
        || data.municipal?.toLowerCase().includes(term)
...
export class ClientsService {
    ...
    // tslint:disable-next-line: variable-name
    private _activities$ = new BehaviorSubject<Clients[]>([]);
    // tslint:disable-next-line: variable-name
  ...
  responceData: Clients[] = [];
     ...
     * Search Method
     */
    private _search(): Observable<{ total: number; activities: Clients[] }> {
