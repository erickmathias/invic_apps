 ...
 */
function sort(data: Elements[], column: string, direction: string): Elements[] {
  if (direction === '') {
    return data;
  } else {
 ...
 */
function matches(data: Elements, term: string, pipe: PipeTransform) {
  return data.id?.toString().toLowerCase().includes(term)
    || data.name?.toLowerCase().includes(term)
    || data.project?.toString().toLowerCase().includes(term)
    || data.std_code.name?.toString().toLowerCase().includes(term)
    || data.steel_type.name?.toString().toLowerCase().includes(term)
    || data.name?.toString().toLowerCase().includes(term)
    || data.density?.toString().toLowerCase().includes(term)
...
export class ElementsService {
  ...
  // tslint:disable-next-line: variable-name
  private _activities$ = new BehaviorSubject<Elements[]>([]);
  // tslint:disable-next-line: variable-name
   ...
   * Search Method
   */
  private _search(): Observable<{ total: number; activities: Elements[] }> {
